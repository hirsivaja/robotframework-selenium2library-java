...
import com.github.markusbernhardt.selenium3library.utils.Python;
...
public class Element extends RunOnFailureKeywordsAdapter {
	...
	public void currentFrameContains(String text, String logLevel) {
		if (!isTextPresent(text)) {
			...
			throw new Selenium3LibraryNonFatalException(
	...
	protected void pageShouldContainElement(String locator, String tag, String message, String logLevel) {
		...
		if (!isElementPresent(locator, tag)) {
			...
			throw new Selenium3LibraryNonFatalException(message);
		} else {
	...
	public void elementShouldBeEnabled(String locator) {
		if (!isEnabled(locator)) {
			throw new Selenium3LibraryNonFatalException(String.format("Element %s is disabled.", locator));
		}
	}
	...
	public void elementShouldBeDisabled(String locator) {
		if (isEnabled(locator)) {
			throw new Selenium3LibraryNonFatalException(String.format("Element %s is enabled.", locator));
		}
	}
	...
	public String getElementAttribute(String attributeLocator) {
		...
		if (elements.size() == 0) {
			throw new Selenium3LibraryNonFatalException(String.format("Element '%s' not found.", parts[0]));
		}
	...
	public String getInnerElementId(String locator, String matchid, int index) {
		...
		if (elements.size() == 0) {
			throw new Selenium3LibraryNonFatalException(String.format("get Inner element '%s' not found.", locator));
		}
	...
	public void mouseDown(String locator) {
		...
		if (elements.size() == 0) {
			throw new Selenium3LibraryNonFatalException(String.format("ERROR: Element %s not found.", locator));
		}
	...
	protected boolean isEnabled(String locator) {
		...
		if (!formElement.isFormElement(element)) {
			throw new Selenium3LibraryNonFatalException(String.format("ERROR: Element %s is not an input.", locator));
		}
