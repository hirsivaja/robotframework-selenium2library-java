package com.github.markusbernhardt.selenium3library.keywords;
...
import com.github.markusbernhardt.selenium3library.RunOnFailureKeywordsAdapter;
import com.github.markusbernhardt.selenium3library.Selenium3LibraryFatalException;
import com.github.markusbernhardt.selenium3library.Selenium3LibraryNonFatalException;
import com.github.markusbernhardt.selenium3library.locators.ElementFinder;
import com.github.markusbernhardt.selenium3library.locators.WindowManager;
import com.github.markusbernhardt.selenium3library.utils.Robotframework;
import com.github.markusbernhardt.selenium3library.utils.WebDriverCache;
import com.github.markusbernhardt.selenium3library.utils.WebDriverCache.SessionIdAliasWebDriverTuple;
...
public class BrowserManagement extends RunOnFailureKeywordsAdapter {
	...
	public String openBrowser(String url, String browserName, String alias, String remoteUrl,
		...
		try {
			...
			throw new Selenium3LibraryFatalException(t);
		}
	}
	...
	public void switchBrowser(String indexOrAlias) {
		try {
			...
			throw new Selenium3LibraryFatalException(String.format("No browser with index or alias '%s' found.",
					indexOrAlias));
		}
	}
	 ...
	 * </table>
	 * <br>
	 * It is also possible to specify the approach Selenium3Library should take
	...
	public void locationShouldBe(String url) {
		...
		if (!actual.equals(url)) {
			throw new Selenium3LibraryNonFatalException(String.format("Location should have been '%s', but was '%s'",
					url, actual));
		}
	...
	public void locationShouldContain(String url) {
		...
		if (!actual.contains(url)) {
			throw new Selenium3LibraryNonFatalException(String.format(
	...
	public void titleShouldBe(String title) {
		...
		if (!actual.equals(title)) {
			throw new Selenium3LibraryNonFatalException(String.format("Title should have been '%s', but was '%s'",
					title, actual));
		}
	...
	public void titleShouldNotBe(String title) {
		String actual = getTitle();
		if (actual.equals(title)) {
			throw new Selenium3LibraryNonFatalException(String.format("Title should not have been '%s', but was '%s'",
					title, actual));
		}
		logging.info(String.format("Page title is '%s'.", title));
	}
	...
	public void titleShouldContain(String title) {
		...
		if (!actual.contains(title)) {
			throw new Selenium3LibraryNonFatalException(String.format("Title should have contained '%s', but was '%s'",
					title, actual));
		}
		logging.info(String.format("Page title is '%s'.", title));
	}
	...
	public void setRemoteWebDriverProxy(String host, String port, String username, String password, String domain,
		...
		try {
			...
			throw new Selenium3LibraryNonFatalException(e.getMessage());
		}
	...
	protected WebDriver createLocalWebDriver(String browserName, DesiredCapabilities desiredCapabilities) {
		if ("ff".equals(browserName) || "firefox".equals(browserName)) {
			...
			try {
				...
				throw new Selenium3LibraryFatalException("Creating " + browserName + " instance failed.", e);
			}
		} else if ("android".equals(browserName)) {
			try {
				return new SelendroidDriver(desiredCapabilities);
			} catch (Exception e) {
				throw new Selenium3LibraryFatalException(e);
			}
		}

		throw new Selenium3LibraryFatalException(browserName + " is not a supported browser.");
	}
	...
	protected void setRemoteWebDriverProxy(HttpCommandExecutor httpCommandExecutor) {
		...
		try {
			...
			throw new Selenium3LibraryFatalException(String.format(
			...
			throw new Selenium3LibraryFatalException(String.format("The field %s does not belong to the given object.",
					fieldName));
		} catch (IllegalAccessException e) {
			throw new Selenium3LibraryFatalException(String.format(
					"The SecurityManager does not allow us to access to the %s field.", fieldName));
		} catch (ClassCastException e) {
			throw new Selenium3LibraryFatalException(String.format("The %s field does not contain a %s.", fieldName,
					className));
		}
	}
